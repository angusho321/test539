#!/usr/bin/env python3
"""
上傳加州Fantasy 5分析結果到 Google Drive
"""
...
def upload_fantasy5_to_google_drive():
    """上傳 fantasy5_prediction_log.xlsx 到 Google Drive"""
    ...
    try:
        # 檢查檔案是否存在
        if not os.path.exists('fantasy5_prediction_log.xlsx'):
            print("❌ fantasy5_prediction_log.xlsx 不存在")
            print("📁 當前目錄檔案:")
            import glob
            files = glob.glob("*")
            for f in files:
        ...
        print(f"📊 準備上傳檔案: fantasy5_prediction_log.xlsx ({file_size} bytes)")
        
        # 優先嘗試更新現有檔案
        original_file_id = os.environ.get('FANTASY5_PREDICTION_LOG_FILE_ID')
        ...
        if original_file_id:
            try:
                ...
                if 'spreadsheet' in target_mime_type or 'google-apps' in target_mime_type:
                    ...
                    media = MediaFileUpload('fantasy5_prediction_log.xlsx',
                ...
                print(f"✅ 成功更新加州Fantasy 5預測檔案: {updated_file.get('name')}")
        ...
        try:
            ...
            filename = f"fantasy5_prediction_log_{today}.xlsx"
            file_metadata = {
                'name': filename
            }
            ...
            print(f"✅ 成功建立新加州Fantasy 5預測檔案: {file.get('name')}")
            print(f"📁 檔案 ID: {file.get('id')}")
            print(f"🔗 檢視連結: {file.get('webViewLink')}")
            print(f"💡 建議將此檔案 ID 新增為 GitHub Secret: FANTASY5_PREDICTION_LOG_FILE_ID")
            
        except Exception as create_error:
            print(f"❌ 建立新檔案也失敗: {str(create_error)}")
            print("💡 解決方案：")
            print("   1. 在 Google Drive 手動建立 fantasy5_prediction_log.xlsx")
            print("   2. 分享給服務帳號（編輯者權限）")
            print("   3. 將檔案 ID 新增為 GitHub Secret: FANTASY5_PREDICTION_LOG_FILE_ID")
...
if __name__ == "__main__":
    upload_fantasy5_to_google_drive()
